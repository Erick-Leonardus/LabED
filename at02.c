...
Color corDaMoeda(TipoMoeda tipo) {
    switch (tipo) {
        ...
        case MOEDA_DIAMANTE: return (Color){102,191,255,255};
...
Moeda *criarMoedas(int quantidade) {
    ...
    for (int i = 0; i < quantidade; i++) {
        ...
        m->raio     = 10.0f;
        //Moeda de diamante - raridade;
        m->tipo     = (TipoMoeda)GetRandomValue(MOEDA_BRONZE, MOEDA_DIAMANTE);
...
int main(void) {
    ...
    while (!WindowShouldClose()) {
        ...
        for (int i = 0; i < TOTAL_MOEDAS; i++) {
            ...
            //Timer de coleta;
            if(m-> coletada) {
                 m->tempo_Coleta += GetFrameTime();
                 if (m->tempo_Coleta >= 3.0f) {
                m->coletada = false;
                 m->pos      = (Vector2){ GetRandomValue(30, LARGURA_JANELA - 30),
                                  GetRandomValue(30, ALTURA_JANELA - 30) };
                                  m->tempo_Coleta = 0;
